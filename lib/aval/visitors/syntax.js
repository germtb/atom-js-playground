	...
	return {
		...
		UnaryExpression: (node, scopes) => {
			...
			if (operator === '!') {
				...
				aval(node.arguments, scopes)
				return undefined
			} else if (operator === 'throw') {
				// TODO
			}
		...
		SwitchStatement: (node, scopes) => {
			// TODO
		},
		DoWhileStatement: (node, scopes) => {
			do {
				aval(node.body, scopes)
			} while (aval(node.test, scopes))
		},
		DoExpression: (node, scopes) => {
			// TODO
		}
	}
}
