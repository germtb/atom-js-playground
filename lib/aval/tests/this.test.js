...
describe('aval', () => {
	describe('this', () => {
		test('evals to global in an arrow scope', () => {
			const expression = '(() => this === global)()'
			const ast = parse(expression)
			expect(aval(ast.program.body[0])).toEqual(true)
		})
		...
		test('evals a this in an arrow in an object to the parent scope this in which it was defined', () => {
			const expression = `
				const obj = {
					something: () => {
						return this
				...
				obj.something() === global
			`

			const ast = parse(expression)
			const scopes = [{}]
			expect(aval(ast.program.body[0], scopes)).toEqual(undefined)
			expect(aval(ast.program.body[1], scopes)).toBe(true)
		})
	})
})
