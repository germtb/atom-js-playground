...
describe('aval', () => {
	context('functions', () => {
		...
		it('evals a recursive function', () => {
			...
			const scope = [{}]
			expect(aval(ast.program.body[0], scope)).to.equal(undefined)
			expect(aval(ast.program.body[1], scope)).to.equal(1)
		})

		it('evals a complex recursive function', () => {
			const expression = `
				const fib = x => {
					if (x <= 2) {
						return 1
			...
			const scope = [{}]
			expect(aval(ast.program.body[0], scope)).to.equal(undefined)
			expect(aval(ast.program.body[1], scope)).to.equal(55)
		})
