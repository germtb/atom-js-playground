...
describe('aval', () => {
	test('evals typeof', () => {
		...
		expect(aval(parse("typeof String('abc')").program.body[0])).toEqual('string')
		...
		expect(aval(parse('typeof undefined').program.body[0])).toEqual(
			'undefined'
		)
		...
		// expect(aval(parse('typeof class C {}').program.body[0])).toEqual(
		// 	'function'
		// )
